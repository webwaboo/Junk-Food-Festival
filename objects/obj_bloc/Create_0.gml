//create Bloc size/color, setup digestion timer, setup gameover timer

// Par défaut, tu peux initialiser des valeurs (elles pourront être modifiées après création)
bloc_taille = 3; // 1 = 16x32, 2 = 32x32, 3 = 48x32
bloc_couleur = "vert"; // "rouge", "vert", "bleu"
bloc_owner = "none"; // Par défaut, ni J1 ni J2


// Choisir le sprite en fonction de la couleur et de la taille
if (bloc_couleur == "rouge") {
    if (bloc_taille == 1) sprite_index = spr_meatball_1;
    else if (bloc_taille == 2) sprite_index = spr_meatball_2;
    else if (bloc_taille == 3) sprite_index = spr_meatball_3;
}
else if (bloc_couleur == "vert") {
    if (bloc_taille == 1) sprite_index = spr_pizza_1;
    else if (bloc_taille == 2) sprite_index = spr_pizza_2;
    else if (bloc_taille == 3) sprite_index = spr_pizza_3;
}
else if (bloc_couleur == "bleu") {
    if (bloc_taille == 1) sprite_index = spr_icecream_1;
    else if (bloc_taille == 2) sprite_index = spr_icecream_2;
    else if (bloc_taille == 3) sprite_index = spr_icecream_3;
}

//var pour la digestion
digestion_timer = -1; // -1 = non actif

//var game over
danger_timer = -1;

global.game_paused = false;
